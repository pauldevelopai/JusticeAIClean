...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = (
    "credentials/podapp-432105-588c6b8fc338.json"
)
...
def load_transcripts():
    transcripts = (
        session.query(Transcription).order_by(Transcription.timestamp.desc()).all()
    )
    transcript_titles = [
        f"{t.filename} - {t.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that summarizes legal transcripts.",
                },
                {
                    "role": "user",
...
def chat_with_database(user_input):
    ...
    all_transcriptions = "\n\n".join(
        [t.transcription for t in st.session_state["transcripts"]]
    )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that answers questions based on the provided legal transcripts.",
                },
                {
                    "role": "assistant",
                    "content": f"The following are transcripts from legal proceedings:\n\n{all_transcriptions}",
                },
...
def main():
    st.title("Justice AI")
    st.write(
        "Upload an audio or video file to transcribe it using Google Speech-to-Text."
    )
    ...
    if st.session_state["transcripts"]:
        selected_transcript = st.sidebar.selectbox(
            "Select a transcript", [""] + st.session_state["transcript_titles"]
        )
        if selected_transcript and selected_transcript != "":
            selected_index = st.session_state["transcript_titles"].index(
                selected_transcript
            )
            ...
            if st.sidebar.button(
                "Summarize Transcript", key=f"summarize_{selected_transcription.id}"
            ):
            ...
            txt_path = save_transcription_as_txt(
                selected_transcription.transcription, selected_transcription.filename
            )
    ...
    uploaded_file = st.file_uploader(
        "Choose an audio or video file", type=["mp3", "wav", "mp4", "mkv", "avi"]
    )
    ...
    if uploaded_file is not None:
        ...
        if st.button("Transcribe Audio/Video"):
            ...
            if transcription:
                ...
                st.text_area(
                    "Transcription", transcription, height=200, key="main_transcription"
                )
    ...
    if (
        ...
        and not st.session_state["transcription_saved"]
    ):
